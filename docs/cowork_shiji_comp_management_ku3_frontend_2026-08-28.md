...
- **付与理由（`note`）は必須。** 空文字・空白のみだと400で拒否される（`detail`は他のAPIと
  同じ通常の文字列。後述の「補足」参照）
...
comp: {
  ...
  /** 付与する（note/email必須。400のdetailは通常の文字列） */
...
- 付与フォーム: メールアドレス入力欄＋理由（note）入力欄＋「付与する」ボタン
  - **note入力欄は空のままでは送信ボタンを無効化する**（`disabled`）。空で送信させない
    通常のフォームバリデーションとして実装すればよい（詳細は下記「補足」参照）。
    プレースホルダで記入例を示す（例:「◯◯社の導入検証用」「△△様のオンボーディング支援」）
...
## 補足: note/emailバリデーションエラーの扱い（[PR #72](https://github.com/Shoichiro12/rakuten-kpi-app/pull/72)で修正済み）

`POST /api/admin/comp-grants` の`email`/`note`検証は当初Pydanticの自動バリデーション
（`field_validator`）で行っており、空文字時に`detail`が配列の422（`{"detail": [{"type": ...,
"msg": "Value error, ..."}]}`）で返っていた。この形のまま`frontend/src/lib/api.ts`の
`parseJson()`（`throw new Error(msg)`）に渡すと`e.message`が壊れた文字列になる問題があったが、
**PR #72で手動チェック方式に書き直し、他のAPI（`consulting.py`/`feedback.py`）と同じ
「400・`detail`は常に文字列」に統一済み。** 現在この経路で配列形式が返ることは無い。

このタスクでは通常の409（Stripe整合エラー）と同様に、`detail`文字列をそのまま
`Notice`（`error`prop）に表示すればよい。**なお、上記3-2の「note/emailが空のときは送信
ボタンを無効化する」という設計自体は変更しない**（正常な入力バリデーションとして引き続き必要。
理由が「422のバグ回避」から「そもそも空で送信させないUX」に変わっただけ）。
...
- `400`: `email`/`note`のバリデーションエラー（空・形式不正）。`detail`は日本語の説明文字列
- `403`: 非管理者、または閲覧モード中
...
納品パッチ `rakuten-kpi_comp-ku3-frontend_main..2e29080.patch`（コミット `2e29080`）を受け入れ。
`git apply --check`→`git am`→`npm run build`／`npx tsc --noEmit`いずれもエラー0、`backend/`は無変更。

**SHA256の記載不一致（記録）**: オーナーへの申し送りでは「SHA256（b77cfe2a…fc07）」と伝わっていたが、
受け取った実ファイルを2通り（PowerShell `Get-FileHash` / `certutil`・`sha256sum`）で独立に算出したところ
`b77cfe2ae52370a06ad64ac6c5beb18825fba2a882af8c5a2444fc1701fc6d07`で両者完全一致した。先頭8桁
（`b77cfe2a`）は一致するが末尾4桁は伝わっていた`fc07`ではなく実際は`6d07`。対象ファイルは上記の
`.patch`ファイルそのもの（frontendのソースファイル自体ではない）。二重計算が一致していること・
パッチ内容が本指示書の仕様と完全に一致していたことから、**内容の破損ではなく、Cowork側の報告値が
オーナーへ伝わる過程での転記ミスの可能性が高い**と判断した。実害は無かったが、今後同様の照合を
行う際は「二重計算で一致すれば内容は信頼できる」という基準で判断してよい。

**フロントの実装自体は指示書どおりで修正不要**（`is_comp`列を追加しない判断・`ConfirmDeleteModal`の
流用・note必須の送信ボタン無効化を含め、全項目が仕様と一致）。ただし受け入れ検証（qa項目5「comp付与→
アカウント一覧の課金状態が「無償提供」に即時反映」）の過程で、**バックエンド側**（`backend/routers/admin_comp.py`。
PR #70で先行マージ済み・本指示書のスコープ外）に、既存アカウント・Subscription行なしへの付与が
管理者自身の契約状態を書き換えてしまう重大なtenancy誤帰属バグが見つかった。修正は
[PR #72](https://github.com/Shoichiro12/rakuten-kpi-app/pull/72)で対応し、そちらのマージ後に
本パッチのPRを作成する（`backend/`を一切変更しないという本指示書の方針は維持したまま）。
