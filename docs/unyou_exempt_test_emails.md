...
反映の確認は、対象メールでサインアップしてカード画面を経ずに `trialing` になることで行う。**⚠️ 2026-09-03以降、`GET /api/billing/diagnose` は管理者（`ADMIN_USER_ID`）限定になった**（一般ユーザーからも見えていたことによる混乱の解消。CLAUDE.md該当行参照）ため、対象メール本人でログインしての確認には使えない。管理者アカウントで確認するなら `/admin` 画面のアカウント一覧（課金状態列）を使うか、Stripe側の状態を見ずアプリ側だけで確認したい場合は「exempt アカウントは Stripe 契約を持たないため『DBに subscription ID がありません』の warn が出るが、これは仕様どおり」という従来の判定基準を管理者が診断パネルで確認する。
