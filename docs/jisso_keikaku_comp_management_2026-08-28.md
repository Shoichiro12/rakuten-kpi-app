...
**✅ 全区切り完了・クローズ（2026-08-31）。** 評定確定（§13）→区切り1・2実装・本番デプロイ確認完了（§14、PR #70/#72/#73）→区切り5先行実施・admin@自己付与（§15）→区切り5完了・demo@移行・機能クローズ（§16）。区切り4（運用文書の正式反映・legal-financeレビュー）のみ未着手（オーナー判断待ち、優先度低）。
...
| Q6 | **必須に変更（推奨「任意」を覆す）。** | 理由（オーナー）: 無償リストは時間が経つと「この人なぜ無償だっけ」が必ず起きる。`EXEMPT_TEST_EMAILS`で「誰をいつ入れたか台帳に残す」運用にしたのと同じ考え方で、付与の瞬間に理由を1行書かせるのが最も安い。短文でよいが必須。§5の`CompGrant.note`を`nullable=False`に変更し、§10のフロント確認ダイアログ（付与フォーム）でも入力必須にする。**空文字時のステータスコードは元々本ドキュメント§7で400と設計していた**が、区切り2の実装時にPydanticの自動バリデーション（`Field(min_length=1)`+`field_validator`）を使ったため、実際には422（`detail`が配列形式）で稼働していた。この422形式はフロントの`parseJson()`が配列をそのまま`Error()`に渡すため画面表示が壊れる問題があり、区切り3（ku3）の受け入れ検証で発覚。手動チェック＋文字列detailに書き直し、**400へ修正して§7の元の設計に一致させた**（[PR #72](https://github.com/Shoichiro12/rakuten-kpi-app/pull/72)。既存API群＝`consulting.py`/`feedback.py`との方式統一も兼ねる） |
...
## 14. 区切り1・2実装完了・本番デプロイ確認・区切り3指示書切り出し（2026-08-28）

**区切り1・2は[PR #70](https://github.com/Shoichiro12/rakuten-kpi-app/pull/70)として実装・マージ済み（マージコミット`70bb2f2`）。** qa検証は本ドキュメント§11の検証コマンド・評定Q4の必須項目を含め12項目全PASS。

**進め方の是正（記録）**: 計画上、区切り1・2は別々の確認ポイントとして設計されていたが、実装セッションが途中承認を挟まず一度に実装した。オーナー指摘を受け、次回以降「区切りを統合したい場合は着手前に理由つきで提案し、了承を得てから進める」運用に是正した。

**検証で見つかり、実装に反映した論点（§2-2/§8の想定外）**: comp状態のユーザーが`POST /api/billing/checkout`を叩くと、既存の`_skip_reason`分岐（EXEMPT_TEST_EMAILS/TRIAL_WITHOUT_CARD）は`status=="comp"`を見ていないため、実際にStripe Checkoutセッションが作られてしまうことが判明。オーナー判断で`create_checkout()`の先頭・Stripe API呼び出し前に`status=="comp"`を409（「無償提供の適用中のため、お支払い手続きは不要です。」）で遮断する処理を追加した。comp解除後は同じユーザーで`create_checkout()`が正常に通ることも確認済み。

**本番デプロイ確認完了（オーナー実施、2026-08-28）**:
1. Renderの自動デプロイでマイグレーションエラー無し（`comp_grants`テーブル作成が正常に通った）
2. 実アカウントでログインし、ダッシュボード・課金表示に回帰が無いことを確認
3. `GET /api/security-status`で`comp_grants`が`protected`一覧に入り`ok:true`（`_enforce_rls_pg()`/`security_status()`はテーブル名をハードコードせず`pg_tables`を全走査する実装のため、他の新規テーブルと同じ経路で自動保護される設計どおりの結果）

**区切り3（フロントエンド）のCowork委任指示書を切り出し済み**: `docs/cowork_shiji_comp_management_ku3_frontend_2026-08-28.md`（管理者閲覧機能の`docs/cowork_shiji_admin_viewer_ku3_frontend_2026-08-28.md`と同じ自己完結・パッチ納品形式）。内容は§10のUI（comp一覧・付与フォームのnote必須・解除・Stripe契約あり409時のエラー表示）と、§7で「フロント設計時に確定」としていた`is_comp`列の要否判断（指示書内で「見送り・`subscription_status=="comp"`と別セクションの一覧で足りる」と確定させた）を含む。

**demo@ureshiru.comのcomp移行方針を変更**: Q2評定では「今回の実装完了時にcompへ移行しenvから外す」としていたが、**区切り3のUI完成後にオーナーが`/admin`画面から実施する方針に変更**。理由は、画面が無い状態でAPIを直接叩いて移行するより、区切り3で作る付与UIをそのまま使ったほうが確実で、かつ実際の運用動線の動作確認も兼ねられるため。したがって**区切り5（本番デプロイ）の作業リストからdemo@移行は外し、区切り3完了後のオーナー作業として扱う**。区切り4（`docs/unyou_exempt_test_emails.md`更新・legal-financeレビュー正式実施）も区切り3完了後に着手する。

---

## 15. 区切り5の先行実施: admin@ureshiru.comへのcomp付与（2026-08-29）

**PR #75（別機能・マスタ削除一括化）の本番デプロイ確認と同じタイミングで、オーナーが本番の`/admin`画面から
`admin@ureshiru.com`自身へcomp付与を実施した**（付与理由「管理者」）。これは`docs/honban_kakunin_comp_management_ku5_2026-08-28.md`
の②〜④が想定する「テスト用メールへ付与→確認→解除」の使い捨てフローとは別物で、**恒久的な実運用の付与**。

**確認できたこと**: 付与対象（`admin@ureshiru.com`）は付与前に`Subscription`行を持っていなかった＝
§8のStripe整合チェックにおける「既存アカウント・`Subscription`行なし」パターンB経路を実際に通過した。
閲覧モードのスクリーンショットで、新規`Subscription`行の作成と`comp`ステータスの反映が正常に確認できた
（クラッシュなし・表示崩れなし）。

**⚠️ 技術的な留保**: PR #72が修正した誤帰属バグ（`create_comp_grant()`が`current_user_id.reset()`後に
commitしていたため、新規`Subscription`行の`user_id`が対象ではなく**付与操作者自身**に書き込まれていた）は、
「対象≠操作者」のケースでのみ検証として成立する。今回は付与者と対象がどちらも`admin@ureshiru.com`
＝**同一アカウントのため、修正後の正しい挙動（対象のIDを書き込む）と旧バグの挙動（操作者のIDを書き込む）が
結果的に同じ値になり、この自己付与単体ではどちらが起きたか区別できない**。

したがって今回の実施で立証できたのは (a) パターンBのコードパスが本番で正常に動作すること、
(b) `comp`ステータスの表示が正しいこと、の2点。**誤帰属バグそのものの本番再発防止確認としては、
対象≠操作者となる別ユーザーへの付与でようやく完全に立証できる**（チェックリスト③の「付与対象以外の
...
## 16. 区切り5完了: demo@ureshiru.comのcomp移行・機能クローズ（2026-08-31）

**実施内容（オーナー実施、順に4点）**:

1. Renderの環境変数`EXEMPT_TEST_EMAILS`を**変数ごと削除**（値を空文字にするのではなく変数自体を
   削除。demo@ureshiru.comが唯一の登録だったため、結果としては既定値の空文字と同義）
2. `/admin`のcomp管理画面から`demo@ureshiru.com`へcompを付与（付与理由「demo」、2026-08-31 15:55）
3. Chrome委任で本番実機を確認: アカウント一覧でdemo@=「無償提供」表示・admin@自身は課金状態が
   変わっていないこと、管理者閲覧モードでdemo@の「請求・プラン」画面に「無償提供中」表示、
   ダッシュボードが402にならず正常表示されること
4. **今回は付与者（admin@）≠対象（demo@）のため、PR #72が修正したtenancy誤帰属バグの本番再発
   防止確認としても完全に立証できたと扱う**（§15のadmin@自己付与では付与者=対象が同一のため
   「対象のIDが書き込まれる正しい挙動」と「旧バグの操作者ID書き込み」が区別できなかった。今回は
   demo@のSubscription行にdemo@のuser_idが正しく書き込まれ、admin@側には一切変化が無いことを
   確認できたため、対象≠操作者ケースでの立証が完了した）

**更新したドキュメント**: 本ファイル（§13の「未実施」記述・§15末尾の「残り」記述）、
`docs/honban_kakunin_comp_management_ku5_2026-08-28.md`（チェックリスト⑤・③該当項目にチェック）、
CLAUDE.md申し送り台帳（該当行）、`docs/unyou_exempt_test_emails.md`（demo@行に削除日を記入、
EXEMPT_TEST_EMAILS運用終了の注記）、`docs/office_map.html`（軍令帳からcomp管理の項目を除去）。

**⚠️ 手順の順序に関する教訓**: 正しい順序は「①新しい無償化の仕組み（comp）を先に有効化する→
②古い仕組み（EXEMPT_TEST_EMAILS）を後で外す」であるべきだが、**今回は逆順**（①EXEMPT変数を
先に削除→②comp付与を後で実施、15:55）になった。EXEMPT削除の再デプロイ完了からcomp付与
反映までの数分間、demo@ureshiru.comは「EXEMPTでもcompでもない＝通常の未契約ユーザー」として
扱われ、`_paid`エンドポイントに対して402が返る状態になっていた可能性がある。**実害は小さい**
（社内デモ用アカウントで、この数分間の実アクセスは未確認・実質的な問題は起きていない）が、
**次回以降、同種の「無償化の仕組みの切り替え」を行うときは必ず新方式を先に有効化してから
旧方式を外す順序を守ること。**
