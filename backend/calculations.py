...
# ─── アクション提案ロジックのしきい値定数（設計ドキュメント2026-08-01）─────────
#
# ROASは2段構え（オーナー確認済み 2026-08-01）:
#   - RPP_ROAS_LINE (100%)  … 損益分岐点割れ＝出血アラート（既存の挙動を維持）
#   - ROAS_PASS_LINE (300%) … 合格ライン。300%未満 AND Limit CPO超過 の複合条件で
#                             KW・商品の停止候補と判定する（3-D'。片方のみは停止しない）
ROAS_PASS_LINE = 300.0

# CVRの汎用ベースライン（3-B）。ベンチマーク解決の最終フォールバック値。
# 2種類のCVRで基準が異なる（別指標として管理する。access_definitions.py の2軸に対応）:
#   - ページ全体CVR（site_uu軸・全流入経路）… 7%が合格ライン
#   - RPP広告経由CVR（rpp_click軸）        … 3〜5%が優秀ライン（判定は下限3%を使う）
PAGE_CVR_BASELINE = 7.0
AD_CVR_BASELINE = 3.0
AD_CVR_BASELINE_LABEL = "3〜5%"

# CTRのデフォルト基準値2%（3-B'。オーナー確認済み）。
# CTRも2段構え: 既存の recommendations.CTR_GOOD_LINE (1%) は緊急アラートのまま維持し、
# 2%は診断分類の「要改善」基準として使う（いきなり基準を厳しくして警告が大量発生する
# 事故を避けるための重ね方。オーナー確認済み 2026-08-01）。
CTR_BASELINE = 2.0

# 新商品フェーズの取り扱い（3-A / 2-B パターン1'）:
#   - 発売から3ヶ月（1クォーター）をデフォルトの様子見期間とする
#   - 新商品フェーズ中のRPP診断は最低クリック母数を50に引き上げる
#     （KW単位50クリックの基準を、KW別データが無い現状は商品粒度に読み替え。
#      オーナー承認済み 2026-08-01）
