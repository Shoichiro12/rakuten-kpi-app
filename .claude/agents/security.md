...
security_index.md の既存行と同じ粒度。結論サマリ → **差分カバレッジ節（必須）** → 前回指摘のフォローアップ表 → 新規指摘（高/中/低、再現手順、影響、対策案） → 実行したコマンドと結果。
静的レビューで確認できないもの（本番envなど）は「範囲外・継続監視」と書く（結論サマリで断定形に格上げしない）。

**差分カバレッジ節**: `.claude/skills/security-check/SKILL.md`「差分カバレッジ節（必須）」参照。
対象範囲の新設・変更ルーター/テーブル/ガード/書き込みエンドポイントを1つずつ列挙し、それぞれ
「どう確認したか」（コードレビューのみ／ローカル実測／本番実測／範囲外）を書く。書き込み系が
複数本ある新設ルーターは、1本だけ実測して残りを「同じ設計だから」で済ませない。
